...
if [ $search -eq 1 ]; then
  ...
  curl -s "https://www.yachtworld.com/yachtworld/search/boat?pageSize=400&page=1&facets=countrySubdivision,make,condition,makeModel,type,class,country,countryRegion,countryCity,fuelType,hullMaterial,hullShape,minYear,maxYear,minMaxPercentilPrices,enginesConfiguration,enginesDriveType,numberOfEngines&fields=id,make,model,year,featureType,specifications.dimensions.lengths.nominal,location.address,aliases,owner.logos,owner.name,owner.rootName,owner.location.address.city,owner.location.address.country,price.hidden,price.type.amount,portalLink,class,media,isOemModel,isCurrentModel,attributes,previousPrice,mediaCount,cpybLogo&useMultiFacetedFacets=true&enableSponsoredSearch=true&enablePremiumPlacement=false&enableOEM=true&locale=en-US&distance=$distance&radius=true&length=$length&uom=ft&sort=recommended&year=$year&fuelType=diesel&multiFacetedBoatTypeClass=\[%22power%22\]&price=$price&currency=USD&advantageSort=1&enableSponsoredSearchExactMatch=true&randomizedSponsoredBoatsSearch=true&randomSponsoredBoatsSize=0" -o $outfile
fi

# Create boats.js
ignore='"Bayliner","Carver","North Pacific","Sea Ray","Mainship","Meridian","Navigator","Formula","Fountain","DeFever","Silverton","Bertram","Jersey","Riviera","Tiara","Tiara Yachts","Beneteau","Post","Viking","Pursuit","Regal","Azimut","Maxum"'
